  ...
  <MySlider title="记录控制">
    <MyCard title="工作流控制">
      ...
      <div class="checkbox-group">
        <MyCheckBox text="点击取消/完成后重启工作流程" font-size="13px" :disabled=true />
    ...
    <MyCard title="EEG">
      <div class="button-group">
        <LabelButton footerText="记录">
          <template #icon>
            <SvgIcon name="record" />
          </template>
        </LabelButton>
        <LabelButton footerText="阻抗">
          <template #icon>
            <SvgIcon name="impedance" />
          </template>
        </LabelButton>
      </div>
    </MyCard>
    <MyCard title="Annotations">
      <label></label>
    </MyCard>
  </MySlider>
</template>
